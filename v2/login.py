...
async def login():
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code != 200:
            print(f"Failed to access the main page: {response.text}")
            exit()
        ...
        if response.status_code != 200:
            print(f"Failed to access the server selection page: {response.text}")
            exit()
        ...
        if response.status_code != 200:
            print(f"Failed to select server: {response.text}")
            exit()
        ...
        if response.status_code != 200:
            print(f"Failed to log in to server: {response.text}")
            exit()
        ...
        if response.status_code != 200:
            print(f"Failed to access the game page: {response.text}")
            exit()

        print(f"Successfully logged in and accessed the game page {response.text}")
